...
def run(cfg_path, out_csv_name = 'mcs_mapped_surgeries.csv', save=True, return_df=False):
    ...
    tfc_df = clean_mapping_df(tfc_df)
    main_specialty_df = clean_mapping_df(main_specialty_df)

    logging.info('Converting mapping df to dictionaries...')

    tfc_mapping = convert_df_to_dict(tfc_df, value_col='treatment_function')
    main_specialty_mapping = convert_df_to_dict(main_specialty_df, value_col='main_specialty')

    logging.info('Mapping tfc/msc codes in maicrobiology df...')
    
    mcs_df = map_codes(mcs_df, tfc_mapping, col = 'treatment_function_code', new_col= 'tfc_desc')
    mcs_df = map_codes(mcs_df, main_specialty_mapping, col = 'main_specialty_code', new_col= 'main_specialty')

    logging.info('Resolving conflicting surgery codes...')

    mcs_df = resolve_conflict_surgeries(mcs_df, 'tfc_desc')
    ...
    if save:
        ...
        mcs_df.to_csv(out_csv, index=False)
    ...
    logging.info(f"Group cardiac surgery and cardiothoracic surgery together?")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        logging.warning("Usage: python map_surgeries.py <configs/config.yaml> <mcs_output.csv>")
        sys.exit(1)
 
    config_path  = sys.argv[1]
    mcs_out_csv = sys.argv[2]

    run(config_path, mcs_out_csv)
