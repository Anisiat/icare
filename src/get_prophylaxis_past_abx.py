...
NON_ANTIBIOTIC_SUFFIXES = ('vir', 'zole', 'ine', 'nan', 'infusion', 'fungin', 'liposomal')
...
def filter_antibiotics(df, medication_col='medication_name_short'):

    # convert all entries into str to handle Nans
    df[medication_col] = df[medication_col].astype(str)

    # Create boolean masks for filtering
    ends_with_suffix = df[medication_col].str.endswith(NON_ANTIBIOTIC_SUFFIXES)
    in_specific_exclude = df[medication_col].isin(ABX_TO_EXCLUDE)
    in_keep = df[medication_col].isin(ABX_TO_KEEP)
...
def get_abx_exposure(
    ...
    administrations_path = PROJECT_ROOT / paths['administrations']
    administrations_df = pd.read_csv(administrations_path)

    # --- restrict mcs ---
    mcs_df = mcs_df[
        config_cols['mcs_analysis'] + config_cols['mcs_time_cols']
    ]
